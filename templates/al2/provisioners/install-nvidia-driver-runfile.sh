...
function resolve_latest_driver_version_from_json() {
  ...
  local LATEST_VERSION=$(jq -e -r --arg prefix "${DRIVER_PREFIX}" '.[$prefix].driver_info[0].release_version' "${TEMP_JSON_FILE}")
...
if gpu-ami-util is-isolated-partition; then
  ...
  # Ref: https://code.amazon.com/reviews/CR-146053793/revisions/2#/details
  rpm_install "opencl-filesystem-1.0-5.el7.noarch.rpm" "ocl-icd-2.2.12-1.el7.x86_64.rpm"
  sudo yum-config-manager --add-repo=${WORKING_DIR}/accelerator/amzn2-nvidia.repo
else
  sudo amazon-linux-extras install epel -y

  # Add NVIDIA repo for nvidia-fabricmanager
  sudo yum-config-manager --add-repo=${NVIDIA_FABRIC_MANAGER_REPO}
fi
