...
function nvidia-open-supported-devices-file() {
  ...
  # Rely on /etc/eks/nvidia-latest-driver-version.txt
  # This code-path would be taken by AL2 GPU AMIs which have drivers installed using a runfile
  KMOD_MAJOR_VERSION=$(cat /etc/eks/nvidia-latest-driver-version.txt | cut -d. -f1)
