...
# which causes the module to still show up in lsmod and fail later checks in the validate.sh provisioner
sudo rmmod -f neuron
...
# Archive and remove the driver
sudo kmod-util archive aws-neuronx
sudo kmod-util remove aws-neuronx
sudo rm -rf /usr/src/aws-neuronx*

# Do not remove the package and versionlock the curent one to avoid unintentional installs of the driver
