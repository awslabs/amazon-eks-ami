...
if [[ "$INSTALL_CONTAINERD_FROM_S3" == "true" ]]; then
  aws s3 cp --region ${BINARY_BUCKET_REGION} s3://${BINARY_BUCKET_NAME}/containerd/${CONTAINERD_VERSION}/containerd-${CONTAINERD_VERSION}-${CONTAINERD_RELEASE_VERSION}.eks.${MACHINE}.rpm /tmp/containerd/
  sudo dnf install -y /tmp/containerd/containerd-${CONTAINERD_VERSION}-1.eks.${MACHINE}.rpm
else
