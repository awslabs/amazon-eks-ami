...
func (b *fsBroker) determineManager(ctx context.Context, interfaceName, mac string) (string, error) {
	...
	if up {
		zap.L().Info("deferring interface management", zap.String("interface", interfaceName), zap.String("manager", networkmanager.ManagerCNI), zap.String("reason", "link already up"))
		return networkmanager.ManagerCNI, nil
	}
	...
	if optedOut {
		...
		if up {
			zap.L().Info("deferring interface management", zap.String("interface", interfaceName), zap.String("manager", networkmanager.ManagerCNI), zap.String("reason", "link brought up during EC2 lookup"))
			return networkmanager.ManagerCNI, nil
		}
		return networkmanager.ManagerSystemd, nil
	}
