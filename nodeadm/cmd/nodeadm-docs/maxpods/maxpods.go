...
func (c *fileCmd) Run(log *zap.Logger, opts *cli.GlobalOptions) error {
	...
	limitMaxPodsFeatureGate := string(api.LimitMaxPods)
	header := []string{"Instance Type", "Classic Max Pods", "Classic Reserved (%)", "Prefix Deleg. Max Pods [1]", "Prefix Deleg. Reserved (%)", "AL2 Legacy Limit", "AL2 Legacy Limit Reserved (%)", fmt.Sprintf("%s Max Pods", limitMaxPodsFeatureGate), fmt.Sprintf("%s Limit Reserved (%%)", limitMaxPodsFeatureGate)}
	writer.WriteHeader(header)

	for _, instanceTypeInfo := range instanceTypes {
		totalMB := instanceTypeInfo.MemoryMB
		classicMaxPods := kubelet.GetClassicMaxPods(instanceTypeInfo.InstanceType, instanceTypeInfo.Region)
		classicReserved := kubelet.GetMemoryMebibytesToReserve(classicMaxPods)
		...
		prefixDelegationReserved := kubelet.GetMemoryMebibytesToReserve(prefixDelegationMaxPods)

		maxPodsLimit := maxpods.GetMaxPodsLimitForMemoryMiB(instanceTypeInfo.MemoryMB)
		limitedReserved := kubelet.GetMemoryMebibytesToReserve(maxPodsLimit)
		...
		legacyLimitReserved := kubelet.GetMemoryMebibytesToReserve(int32(legacyMaxPodsLimit))
