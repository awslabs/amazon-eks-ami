...
// number of IPs it could allocate. ipamd is typically ran as part of a pod, so this will usually
// error if invoked at initial bootstrap
func InterrogateMaxPods() (int32, error) {
	if maxIPs, err := ipamd.GetMaxAllocatableIPs(); err != nil {
		return 0, err
	} else if maxIPs <= 0 {
