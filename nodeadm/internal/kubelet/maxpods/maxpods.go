...
type MaxPodsRetrievalFunction func() (int32, error)
...
func projectMaxPodsFromMemoryMiBReserved(memoryMiB int) int32 {
	return int32((memoryMiB - 255) / 11)
}

// GetMaxPodsLimitForMemoryMiB returns a max pods limit value based on memory reserved
func GetMaxPodsLimitForMemoryMiB(memoryMiB int) int32 {
	baseReserved := 299
	percentsPerRange := []int{5, 15, 10, 5}
	memoryBands := []int{512, 1024, 2048, 4096, 32768}
	maxReservable := baseReserved + util.GetAmountFromPercentageBands(percentsPerRange, memoryBands, memoryMiB)
...
func ApplyLimit(maxPods int32) int32 {
	...
	maxPodsLimit := GetMaxPodsLimitForMemoryMiB(int(totalMemory))
