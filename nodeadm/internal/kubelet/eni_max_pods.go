...
func PollInterrogateMaxPods(ctx context.Context, maxDuration time.Duration, backoff time.Duration) (int32, error) {
	...
	for {
		maxPods, err := interrogateMaxPods(timedCtx)
		if err != nil {
			if ipamd.IsErrIPAMDNotReady(err) {
				...
				return 0, fmt.Errorf("got error while interrogating max pods from the CNI: %v", err)
			}
		} else {
			return maxPods, err
		}
...
func interrogateMaxPods(ctx context.Context) (int32, error) {
	maxIPs, err := ipamd.GetMaxAllocatableIPs(ctx)
	if err != nil {
		return 0, err
	} else if maxIPs <= 0 {
