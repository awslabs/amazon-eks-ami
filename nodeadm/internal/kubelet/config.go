...
func (k *kubelet) GenerateDefaultKubeletConfig(cfg *api.NodeConfig) (*KubeletConfig, error) {
	...
	kubeletConfig.WithReservedResources(GetDefaultMaxPods(cfg.Status.Instance.Region, cfg.Status.Instance.Type))
...
func NewConfigFromDropinFile(filename string) (KubeletConfig, error) {
	bytes, err := os.ReadFile(filepath.Join(dropinConfigDir, filename))
	if err != nil {
...
func DropinConfigExists(filename string) (bool, error) {
...
func WriteDropinConfigFile(config interface{}, filename string) error {
	...
	userKubeletConfigMap, err := util.Merge(getConfigV1Beta1TypeMeta(), config, json.Marshal, json.Unmarshal)
	if err != nil {
		return err
	}
