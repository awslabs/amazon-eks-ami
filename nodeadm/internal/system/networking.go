...
func (a *networkingAspect) ensureAdditionalInterfaceNetworkConfigurations(cfg *api.NodeConfig) error {
	for _, iface := range cfg.Status.Instance.NetworkInterfaces {
		if iface.NetworkCardIndex == 0 {
			continue
		}

		networkInterfaceConfName := fmt.Sprintf("70-%s.network", iface.InterfaceId)
		...
		vars := additionalInterfaceTemplateVariables{
			PermanentMACAddress: iface.MAC,
			IpV4Address:         iface.IpV4Address,
			IpV4Subnet:          iface.IpV4Subnet,
			IpV6Address:         iface.IpV6Address,
			IpV6Subnet:          iface.IpV6Subnet,
			// https://github.com/amazonlinux/amazon-ec2-net-utils/blob/80ce62f654da3cb752be2dfe55279b86d01042e0/lib/lib.sh#L270
			RouteTableId: 10_000 + 100*iface.NetworkCardIndex + iface.DeviceNumber,
			// https://github.com/amazonlinux/amazon-ec2-net-utils/blob/80ce62f654da3cb752be2dfe55279b86d01042e0/lib/lib.sh#L269C36-L269C66
			RouteTableMetric: 512 + 100*iface.NetworkCardIndex + iface.DeviceNumber,
		}
		...
		zap.L().Sugar().Infof("Multicard instance found, configuring card with index: %d, network file: %s", iface.NetworkCardIndex, networkInterfaceConfPathName)
	}
	return nil
}
