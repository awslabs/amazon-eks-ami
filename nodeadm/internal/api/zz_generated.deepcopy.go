...
func (in *EnvironmentOptions) DeepCopyInto(out *EnvironmentOptions) {
	*out = *in
	...
	if in.SystemdKubelet != nil {
		in, out := &in.SystemdKubelet, &out.SystemdKubelet
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
	}
	if in.SystemdContainerd != nil {
		in, out := &in.SystemdContainerd, &out.SystemdContainerd
		*out = make(map[string]string, len(*in))
		for key, val := range *in {
			(*out)[key] = val
		}
