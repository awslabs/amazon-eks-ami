...
Tag the ENI before attachment. If EC2 has not exposed the ENI or its ownership tags yet, nodeadm leaves the interface untouched and retries without caching a decision. Interfaces already administratively up are left with their existing manager. Untagged interfaces remain pending until a manager brings them up or ownership tags become visible; set `node.k8s.amazonaws.com/no_manage=false` to explicitly delegate a down interface to CNI. In-place retagging of an already managed interface is not supported.

Each EC2 lookup is bounded to ten seconds, including SDK retries. The systemd unit retries failures after five seconds without a start limit, allowing recovery after API outages or node-role IAM corrections. This also applies to other failures of the unit when the feature is disabled. The node must be able to reach the regional EC2 endpoint (directly, through a VPC endpoint, or through its configured proxy).

An uncached, down ENI with unknown ownership triggers another EC2 lookup on each attempt. With fast responses that still lack the tag, this approaches twelve lookups per minute per pending ENI; SDK retries on API errors can add requests. The link-state checks are best effort: they do not prevent another manager from bringing up the interface after the last check, before network configuration is applied.
